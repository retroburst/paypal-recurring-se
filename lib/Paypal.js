(function() {
  ...
  Paypal = (function() {
    ...
    Paypal.prototype.makeAPIrequest = function(params, callback) {
      axios.post(this.endpointUrl, params, { headers: { 'Content-Type': 'application/x-www-form-urlencoded' } }).then(response => {
          if (response.status !== 200) {
            var parsed = querystring.parse(response.data);
            return callback(parsed ? parsed : response.status);
          }
          return callback(null, querystring.parse(response.data));
        }).catch(error => {
